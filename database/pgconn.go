...
func CreatePGConn() (*gorm.DB, error) {
	dsn := fmt.Sprintf("host=localhost user=postgres password=%s dbname=zcelero port=5432 sslmode=disable TimeZone=America/Sao_Paulo", os.Getenv("POSTGRES_PASSWORD"))
